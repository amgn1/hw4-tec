...
INPUT_PDF = 'c:\\Users\\dimaf\\Desktop\\progadima\\hw_4.pdf'  # название pdf-файла с началом д\з, все рисунки добавляются к нему
INPUT_RAW = {7:'c:\\Users\\dimaf\\Desktop\\progadima\\7.raw', 8:'c:\\Users\\dimaf\\Desktop\\progadima\\8.raw', 9:'c:\\Users\\dimaf\\Desktop\\progadima\\9.raw', 10:'c:\\Users\\dimaf\\Desktop\\progadima\\10.raw'}  # список raw-файлов по которым будут строиться графики
...
def raw2plot(file_number: int, n: int, d: int):
    name = INPUT_RAW[file_number]
    ...
    i_data = i_data[::-1]
    _, ir, iphi = zip(*i_data)
    _, vr, vphi = zip(*v_data)
    tick_names = [str((i*d) / N) + " мкм" for i in range(0, 11)]
    ...
    i_plot.set_xticks(range(len(tick_names)))
    i_plot.set_xticklabels(tick_names)
    i_plot.tick_params(axis='x', labelrotation=45)
    ...
    v_plot.set_xticks(range(len(tick_names)))
    v_plot.set_xticklabels(tick_names)
    v_plot.tick_params(axis='x', labelrotation=45)

    fig.suptitle(MODES[file_number], fontsize=16)
    fig.tight_layout()
    ...
    plt.savefig(str(file_number) + '.pdf')


d = 4000 # В микрометрах, как оно дано в условии
N = 10 # кол-во, секций
[raw2plot(i, N, d) for i in range(7, 11)]
...
merger.write("c:\\Users\\dimaf\\Desktop\\progadima\\hw_4_with_plt.pdf")
merger.close()